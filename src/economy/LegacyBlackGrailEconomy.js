...
export class LegacyBlackGrailEconomy extends FactionEconomyStrategy {
  update(dt){const buildings=this.simulation.buildings,points=this.simulation.points;this.simulation.resources[this.faction]+=dt*(BALANCE.economy.baseIncome+buildings.filter(b=>b.f===this.faction&&b.hp>0&&b.progress===1&&b.type==='supply').length*BALANCE.economy.supplyIncome+points.filter(p=>p.owner===this.faction).length*BALANCE.economy.controlIncome);}
