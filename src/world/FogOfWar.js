...
export class FogOfWar {
  ...
  update(dt,force=false){this.timer-=dt;if(!force&&this.timer>0)return;this.timer=BALANCE.fog.refresh;for(let faction=0;faction<2;faction++){const grid=this.grids[faction];for(let i=0;i<grid.length;i++)if(grid[i]===VISIBILITY.VISIBLE)grid[i]=VISIBILITY.EXPLORED;for(const squad of this.simulation.squads)if(squad.f===faction&&squad.hp>0){const definition=getUnitDefinition(squad.type);this.reveal(grid,squad.x,squad.z,definition?.vision??(definition?.combatUnit?BALANCE.fog.combatVision:BALANCE.fog.supportVision));}for(const building of this.simulation.buildings)if(building.f===faction&&building.hp>0&&building.progress===1)this.reveal(grid,building.x,building.z,building.type==='observation'?BALANCE.fog.observationVision:BALANCE.fog.buildingVision);}}
}
