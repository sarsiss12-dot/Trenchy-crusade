...
export const pruneSelection=(sim,selected)=>{for(const id of selected){const entity=[...sim.squads,...sim.buildings].find(item=>item.id===id);if(!entity||entity.hp<=0||entity.f!==sim.player&&!sim.fogOfWar?.visible(sim.player,entity))selected.delete(id);}return selected;};
export const squadGroundStatus=(sim,squad)=>{const fieldCover=coverAt(sim.buildings,squad.x,squad.z);if(fieldCover<1)return 'Saha savunması: %'+Math.round((1-fieldCover)*100)+' koruma';const kind=terrain(squad.x,squad.z);return({trench:'Siper: %42 koruma',forest:'Orman: %22 koruma',mud:'Çamur: yavaş ilerleme',bridge:'Geçit',land:'Açık arazi'}[kind]||'Açık arazi');};
export class HUD {
  constructor(get){this.get=get;}
  update(sim,selected){
    const $=this.get;pruneSelection(sim,selected);
    ...
    else if(squads.length){$('selectionType').textContent=FACTION_DEFINITIONS[squads[0].f].name.toUpperCase()+' / MANGA';$('selectionName').textContent=squads.length===1?getUnitDefinition(squads[0].type).names[squads[0].f]:squads.length+' manga seçili';const alive=squads.reduce((sum,squad)=>sum+Math.ceil(squad.hp/getUnitDefinition(squad.type).hp),0),engineers=squads.filter(squad=>squad.type==='engineer');$('selectionInfo').textContent=engineers.length?alive+' mühendis · '+engineers.map(squad=>squad.worker.state.replaceAll('_',' ')+(squad.worker.cargo?' · '+squad.worker.cargo+' '+squad.worker.cargoType:'')).join(' / '):alive+' asker · '+Math.ceil(squads.reduce((sum,squad)=>sum+squad.hp,0))+' CAN · '+squadGroundStatus(sim,squads[0]);}
